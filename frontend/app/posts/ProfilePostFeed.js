...
export function ProfilePostFeed() {
    ...
    const fetchUserPosts = useCallback(async (loadMore = false) => {
        ...
        try {
            ...
            const url = `http://localhost:8080/api/profile?offset=${currentOffset}`;
            ...
            if (loadMore) {
                ...
                const uniqueNewPosts = userPosts.filter(post => !existingPostIds.has(post.Pid));
    ...
    if (error) {
        return (
            <div className={styles.errorMessage}>
                {error}
                <button onClick={fetchUserPosts} className={styles.retryButton}>
                    Try Again
                </button>
            </div>
        );
